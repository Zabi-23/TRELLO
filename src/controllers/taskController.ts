...
const taskSchema = Joi.object({
    ...
    status: Joi.string().valid('to-do', 'in progress', 'blocked', 'done').optional(),
...
export const updateTaskStatus = async (req: Request, res: Response) => {
    ...
    const { error } = Joi.object({ status: Joi.string().valid('to-do', 'in progress', 'blocked', 'done').required() }).validate({ status });
    if (error) {
        return res.status(400).json({ errors: error.details.map(d => d.message) });
    }
